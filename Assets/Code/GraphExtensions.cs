        ...
        while (open_set.Count > 0)
        {
            ...
            while (neighbors_set.Count > 0)
            {
                ...
                foreach (Node shared_neighbor in shared_neighbors)
                {
                    ...
                    if (node_to_neighbor.RadiansBetween(node_to_shared_neighbor) < minimum_angle)
                    {
...
public static class NodeExtensions
{
                                 ...
                                 GraphUtility.Metric metric)
    {
        ...
        while (open_set.Count > 0)
        {
            Node node = open_set.MinElement(node_ => GetPathCostEstimate(node_));
            open_set.Remove(node);
            closed_set.Add(node);
